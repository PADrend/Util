...
LoggerTime::LoggerTime(std::ostream & outputStream) :
	Logger(outputStream) {

}
LoggerTime::~LoggerTime() = default;
void LoggerTime::log(const Action & action) {
	output << action.getString(ATTR_description) << ": " << ((action.getDouble(ATTR_timeEnd) - action.getDouble(ATTR_timeBegin)) * 1.0e-6) << "\n";
}
