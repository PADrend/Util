...
\note if bytesPerComponent is 1, it can be assumed that the pixel data is stored as bytes.
\note if bytesPerComponent is 4, it can be assumed that the pixel data is stored as floats.
\note if a color component (r,g,b or a) is not set, its byteOffset is set to the constant NONE. */
class PixelFormat {
	uint8_t bytesPerComponent;
	uint8_t byteOffset_r;
	uint8_t byteOffset_g;
	uint8_t byteOffset_b;
	uint8_t byteOffset_a;
...
public:
	...
	PixelFormat( uint8_t _bytesPerComponent,uint8_t _byteOffset_r,uint8_t _byteOffset_g,uint8_t _byteOffset_b,uint8_t _byteOffset_a=NONE ) :
			bytesPerComponent(_bytesPerComponent), byteOffset_r(_byteOffset_r), byteOffset_g(_byteOffset_g), byteOffset_b(_byteOffset_b), byteOffset_a(_byteOffset_a),
			numComponents( (byteOffset_r==NONE ? 0 : 1) + (byteOffset_g==NONE ? 0 : 1) + (byteOffset_b==NONE ? 0 : 1) + (byteOffset_a==NONE ? 0 : 1) ),
			bytesPerPixel( numComponents * _bytesPerComponent ){
	}
	bool operator==(const PixelFormat & other)const{
		return this==&other || (bytesPerComponent == other.bytesPerComponent &&
	...
	bool isValid()const					{	return bytesPerComponent>0;	}
	...
	static const PixelFormat UNKNOWN;		// bytesPerComponent is 0. No direct pixel access is possible.
};
