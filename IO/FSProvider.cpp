...
//! ---|> AbstractFSProvider
std::ostream * FSProvider::openForWriting(const FileName & filename) {
	std::ofstream * stream = new std::ofstream(filename.getPath().c_str(), std::ios_base::out | std::ios_base::binary | std::ios_base::trunc);
	if(!stream->good()) {
		delete stream;
		return nullptr;
	}
